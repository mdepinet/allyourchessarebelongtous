...
public class OtherNameGenerator extends NameGenerator {
    ...
    public OtherNameGenerator(String fileName) throws IOException{
    	super();
    ...
    public String compose(int syls){
	    String name = "";
    	if(pre.size()==0 && sur.size()==0 && mid.size()>0) {
    		int[] b = new int[syls];
    		for(int i = 0; i < b.length; i++)
    			b[i] = (int)(Math.random() * mid.size());
    		for(int i = 0; i < b.length; i++)
    			name = name + upper(mid.get(b[i])) + " ";
    		name.trim();
    	}
    	else {
			int number = syls;
